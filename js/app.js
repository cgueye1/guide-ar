...
function paintArButton(step){
  ...
  // Pas de son piloté par la page sur iOS : dès que Quick Look s'ouvre,
  // la session audio d'ARKit interrompt celle de la page. Le son doit être
  // embarqué dans le .usdz (voir tools/usdz-add-audio.py), Quick Look le
  // joue alors lui-même au moment où le modèle est posé.
...
/* ── son de la scène AR ───────────────────────────────────────
   ...
   iOS : rien ici. La session audio d'ARKit interrompt celle de la page dès
   l'ouverture de Quick Look, un son lancé depuis le web est donc coupé
   aussitôt. L'audio doit vivre dans le .usdz, sous forme de prim
   UsdMediaSpatialAudio — voir tools/usdz-add-audio.py.
